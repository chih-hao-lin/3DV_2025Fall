...
def train_static(cfg, pnerf:PACNeRF, optimizer, start, max_iter, rays_o_all, rays_d_all, viewdirs_all, rgb_all, ray_mask_all):
...
def train_velocity(cfg, pnerf:PACNeRF, start, rays_o_all, rays_d_all, viewdirs_all, rgb_all, ray_mask_all, point_gt=None):
...
def train_dynamic(cfg, pnerf:PACNeRF, optimizer, start, rays_o_all, rays_d_all, viewdirs_all, rgb_all, ray_mask_all, point_gt=None):
    pnerf.dynamic_observer.particle.deactivate_all()
    ...
    for stage, frame_iter in enumerate(zip([cfg["hit_frame"] + cfg["warmup_step"], rgb_all.shape[1]], [50, 100])):
        if stage >= start:
            ...
            for i in range(iter):
                ...
                optimizer.step()
                print("init_mu:", pnerf.init_mu.mean().item(), "init_lam:", pnerf.init_lam.mean().item())
                torch.save({
                    'epoch': stage + 1,
                    'model_state_dict': pnerf.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
