...
class TqdmHook(tf.train.SessionRunHook):
  """Show a tqdm progress bar driven by global_step."""
  def __init__(self, total_steps, every_n_steps=1):
    self.total_steps = int(total_steps)
    self.every_n_steps = int(every_n_steps)

  def begin(self):
    self._last_gs = 0
    self._bar = tqdm(total=self.total_steps, dynamic_ncols=True, smoothing=0.1)

  def before_run(self, run_context):
    gs = tf.train.get_global_step()
    return tf.train.SessionRunArgs({'gs': gs})

  def after_run(self, run_context, run_values):
    gs = int(run_values.results['gs']) if run_values.results['gs'] is not None else self._last_gs
    if gs - self._last_gs >= self.every_n_steps:
      delta = max(0, gs - self._last_gs)
      self._bar.update(delta)
      self._last_gs = gs

  def end(self, session):
    if self._last_gs < self.total_steps:
      self._bar.update(self.total_steps - self._last_gs)
    self._bar.close()
...
def main(_):
  ...
  if FLAGS.mode in ['train', 'eval']:
    ...
    hooks=[TqdmHook(total_steps=FLAGS.num_steps, every_n_steps=1)])
    else:
