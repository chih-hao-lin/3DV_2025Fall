...
class RealEstate10KBaseVideoDataset(BaseVideoDataset):
    ...
    def setup(self) -> None:
        # Use the proper VideoTransform to ensure consistent video dimensions
        self.transform = self.build_transform()

    def video_path_to_preprocessed_path(self, video_path: Path) -> Path:
        # The video_path already contains the full correct path from metadata
        # We just need to return it as-is since the videos are already in the right location
        return video_path
...
class RealEstate10KAdvancedVideoDataset(
    ...
    def load_cond(
        ...
        # Check if there's a video path in metadata that indicates the data structure
        # If the video path contains specific directory structure, use that to determine pose path
        if "train" in str(path) or "training" in str(path):
            # Video is in train/training directory, so poses are in train_poses
            actual_split = "train"
            pose_path = self.save_dir / actual_split / f"{actual_split}_poses" / f"{path.stem}.pt"
        elif "test" in str(path):
            # Video is in test directory, so poses are in test_poses  
            actual_split = "test"
            pose_path = self.save_dir / actual_split / f"{actual_split}_poses" / f"{path.stem}.pt"
        else:
            # Fallback to original logic based on split
            if self.split == "training":
                actual_split = "train"
            elif self.split == "validation":
                actual_split = "test"  # validation uses test data
            else:
                actual_split = self.split
                
            pose_path = self.save_dir / actual_split / f"{actual_split}_poses" / f"{path.stem}.pt"
        cond = torch.load(pose_path, weights_only=False)[start_frame:end_frame]
        return cond
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Calculate frame skip based on split
        if self.split == "training":
        ...
        # Calculate end frame to ensure we get exactly max_frames after frame skipping
        end_frame = start_frame + (self.cfg.max_frames - 1) * frame_skip + 1
        
        # Load video and condition
        ...
        current_frames = len(video)
        if current_frames < self.cfg.max_frames:
            # Pad if we have fewer frames than needed
            pad_len = self.cfg.max_frames - current_frames
        ...
        elif current_frames > self.cfg.max_frames:
            # Truncate if we have more frames than needed
            video = video[:self.cfg.max_frames]
            cond = cond[:self.cfg.max_frames]
            nonterminal = torch.ones(self.cfg.max_frames, dtype=torch.bool)
        else:
