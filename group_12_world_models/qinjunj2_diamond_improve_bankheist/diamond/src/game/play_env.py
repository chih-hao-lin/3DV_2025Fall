...
class PlayEnv:
    def __init__(
        ...
        self.last_action = None  # Store last sampled action for repetition
        self.repeat_counter = 0  # Counter for action repetition
        
        # CSV logging for episode statistics
        self.csv_path = Path("episode_stats.csv")
        self.csv_file = None
    ...
    def _init_csv(self) -> None:
        ...
        if not self.csv_path.exists():
            ...
            self.csv_writer.writerow(['return', 'episode_length'])
            self.csv_file.flush()

    def _log_episode_to_csv(self) -> None:
        """Log episode return and length to CSV."""
        if self.csv_writer is None:
            self._init_csv()
        self.csv_writer.writerow([self.return_, self.t])
        self.csv_file.flush()
    ...
    def step(self, act: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Dict[str, Any]]:
        if self.is_human_player:
            # Human player: always use provided action, reset repetition counter
            act = torch.tensor([act], device=self.agent.device)
            self.last_action = act
            self.repeat_counter = 0
        else:
            # Policy player: sample new action only every action_repeat frames
            if self.repeat_counter == 0 or self.last_action is None:
                ...
                act = dst.sample()
                self.last_action = act
                self.repeat_counter = self.action_repeat
                entropy = dst.entropy() / math.log(2)
            else:
                # Repeat last action, decrement counter
                act = self.last_action
                self.repeat_counter -= 1
                entropy = None  # No new action sampled, so no entropy
            
            entropy = None if self.is_human_player else f"{entropy.item():.2f}" if isinstance(entropy, Tensor) else entropy
            value = None if self.is_human_player else f"{value.item():.2f}" if 'value' in locals() else "—"
