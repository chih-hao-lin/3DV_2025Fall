...
class DenoiserConfig:
    ...
    history_encoder: Optional[HistoryEncoderConfig] = None
...
class Denoiser(nn.Module):
    def __init__(self, cfg: DenoiserConfig) -> None:
        ...
        # Initialize history encoder if needed
        if cfg.use_latent_history and cfg.history_encoder is not None:
            num_actions = cfg.inner_model.num_actions
            self.history_encoder = HistoryEncoder(cfg.history_encoder, num_actions=num_actions)
            # Ensure inner_model is also configured for latent conditioning
            self.inner_model.use_latent_history = True
    ...
    def compute_model_output(self, noisy_next_obs: Tensor, obs: Tensor, act: Tensor, cs: Conditioners, latent_history: Optional[Tensor] = None) -> Tensor:
        rescaled_obs = obs / self.cfg.sigma_data
        rescaled_noise = noisy_next_obs * cs.c_in
        return self.inner_model(rescaled_noise, cs.c_noise, rescaled_obs, act, latent_history)
    ...
    @torch.no_grad()
    def denoise(self, noisy_next_obs: Tensor, sigma: Tensor, obs: Tensor, act: Tensor, latent_history: Optional[Tensor] = None) -> Tensor:
        cs = self.compute_conditioners(sigma)
        model_output = self.compute_model_output(noisy_next_obs, obs, act, cs, latent_history)
    ...
    def forward(self, batch: Batch) -> LossAndLogs:
        ...
        for i in range(seq_length):
            ...
            b, t, c, h, w = obs.shape
            obs_reshaped = obs.reshape(b, t * c, h, w)

            # Encode history to latent if available
            latent_history = None
            if self.history_encoder is not None:
                latent_history = self.history_encoder(obs, act)

            sigma = self.sample_sigma_training(b, self.device)
            noisy_next_obs = self.apply_noise(next_obs, sigma, self.cfg.sigma_offset_noise)

            cs = self.compute_conditioners(sigma)
            model_output = self.compute_model_output(noisy_next_obs, obs_reshaped, act, cs, latent_history)
