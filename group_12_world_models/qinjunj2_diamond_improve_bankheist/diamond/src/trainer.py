...
class Trainer(StateDictMixin):
    def __init__(self, cfg: DictConfig, root_dir: Path) -> None:
        ...
        # Sort training episodes by reward (ensures sample_weights bias towards higher rewards)
        if self._rank == 0 and not cfg.common.resume:
            self.train_dataset.sort_episodes_by_reward()
    ...
    def load_state_checkpoint(self) -> None:
        print(f"Loading checkpoint from {self._path_state_ckpt}")
