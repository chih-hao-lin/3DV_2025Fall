...
@torch.no_grad()
def model_forward_wrapper(all_models, curr_obs, curr_delta, num_timesteps, latent_size, device, num_cond, num_goals=1, rel_t=None, progress=False, return_latents=False):
    # Added return_latents argument above ^
    ...
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
        ...
        # --- HACK START ---
        if return_latents:
            # Return raw latents (4x32x32) instead of decoding to pixels (3x256x256)
            # This is ~10x faster and uses far less memory
            return samples
        # --- HACK END ---
