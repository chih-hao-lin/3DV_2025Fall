...
# moviepy 2.x에서는 editor 모듈이 없고 직접 import
try:
...
def all_edges_white(img):
    ...
    # CloudRendering may return RGBA format
    if len(img.shape) == 3 and img.shape[-1] == 4:
        # RGBA: check only RGB channels, ignore alpha
    ...
    # Check top edge
    if not np.all(np.all(img_rgb[0, :] == white, axis=-1)):
        return False
    # Check bottom edge
    if not np.all(np.all(img_rgb[-1, :] == white, axis=-1)):
        return False
    # Check left edge
    if not np.all(np.all(img_rgb[:, 0] == white, axis=-1)):
        return False
...
def get_top_down_frame(scene, objaverse_asset_dir, width=1024, height=1024):
    controller = Controller(
        ...
        height=height,
        fieldOfView=90,
        platform=CloudRendering,  # Use CloudRendering for headless environments
