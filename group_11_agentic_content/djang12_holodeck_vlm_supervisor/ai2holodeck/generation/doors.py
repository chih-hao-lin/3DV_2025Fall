...
class DoorGenerator:
    ...
    def load_features(self):
        ...
        try:
            ...
            if isinstance(self.door_feature_clip, torch.Tensor):
                self.door_feature_clip = self.door_feature_clip.to(device)
        except:
    ...
    def select_door(self, door_type, door_size, query):
        ...
        with torch.no_grad():
            tokenized_query = self.clip_tokenizer([query])
            # Move tokenized input to device
            if hasattr(tokenized_query, 'to'):
                tokenized_query = tokenized_query.to(device)
            elif isinstance(tokenized_query, dict):
                tokenized_query = {k: v.to(device) if hasattr(v, 'to') else v 
                                   for k, v in tokenized_query.items()}
            
            query_feature_clip = self.clip_model.encode_text(tokenized_query)
