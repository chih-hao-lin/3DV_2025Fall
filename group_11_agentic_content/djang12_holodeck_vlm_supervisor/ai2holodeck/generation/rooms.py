...
class MaterialSelector:
    ...
    def load_features(self):
        ...
        try:
            ...
            if isinstance(self.material_feature_clip, torch.Tensor):
                self.material_feature_clip = self.material_feature_clip.to(device)
        except:
            ...
            for material in tqdm(self.selected_materials):
                ...
                ).unsqueeze(0).to(device)  # Move image to device
                with torch.no_grad():
        ...
        try:
            ...
            # Move to same device as model
            if isinstance(self.color_feature_clip, torch.Tensor):
                self.color_feature_clip = self.color_feature_clip.to(device)
        except:
            print("Precompute text features for colors...")
            with torch.no_grad():
                tokenized_colors = self.clip_tokenizer(self.colors)
                # Move tokenized input to device
                if hasattr(tokenized_colors, 'to'):
                    tokenized_colors = tokenized_colors.to(device)
                elif isinstance(tokenized_colors, dict):
                    tokenized_colors = {k: v.to(device) if hasattr(v, 'to') else v 
                                       for k, v in tokenized_colors.items()}
                self.color_feature_clip = self.clip_model.encode_text(tokenized_colors)
    ...
    def match_material(self, queries, topk=5):
        ...
        with torch.no_grad():
            tokenized_queries = self.clip_tokenizer(queries)
            # Move tokenized input to device
            if hasattr(tokenized_queries, 'to'):
                tokenized_queries = tokenized_queries.to(device)
            elif isinstance(tokenized_queries, dict):
                tokenized_queries = {k: v.to(device) if hasattr(v, 'to') else v 
                                     for k, v in tokenized_queries.items()}
            
            query_feature_clip = self.clip_model.encode_text(tokenized_queries)
        ...
        string_similarity = torch.tensor(
            ...
            device=device  # Move to same device as clip_similarity
        )
