...
class ObjectSelector:
    def __init__(self, object_retriever: ObjathorRetriever, llm: OpenAI):
        ...
        self.multiprocessing = False  # Disabled due to CUDA tensor incompatibility with fork
    ...
    def get_floor_objects(
        ...
        total_objects = len(floor_object_list)
        for idx, floor_object in enumerate(floor_object_list, 1):
            ...
            print(f"[Progress] Processing floor object {idx}/{total_objects}: {object_type} ({object_description})...")
            ...
            print(f"  -> Searching database for '{object_type}'...")
    ...
    def get_wall_objects(
        ...
        total_wall_objects = len(wall_object_list)
        for idx, wall_object in enumerate(wall_object_list, 1):
            ...
            print(f"[Progress] Processing wall object {idx}/{total_wall_objects}: {object_type} ({object_description})...")
            print(f"  -> Searching database for '{object_type}'...")
            candidates = self.object_retriever.retrieve(
                [f"a 3D model of {object_type}, {object_description}"],
