...
class ObjathorRetriever:
    def __init__(
        ...
        # Check device and move features to GPU if available
        ...
        if self.device == "cuda":
            print(f"[ObjathorRetriever] Moving features to GPU ({torch.cuda.get_device_name(0)})...")
            self.clip_features = self.clip_features.to(self.device)
        else:
            print(f"[ObjathorRetriever] Using CPU (GPU not available)")
    ...
    def retrieve(self, queries, threshold=28):
        # Ensure models and features are on the same device
        device = next(self.clip_model.parameters()).device
        
        with torch.no_grad():
            # Tokenize and move to device
            tokenized_queries = self.clip_tokenizer(queries)
            # open_clip tokenizer returns tensors that need to be moved to device
        ...
        # Ensure sbert features are on the same device
        if query_feature_sbert.device != self.sbert_features.device:
            self.sbert_features = self.sbert_features.to(query_feature_sbert.device)
