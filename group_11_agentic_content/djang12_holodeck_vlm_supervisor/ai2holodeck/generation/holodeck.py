...
def _simplify_scene(scene: Dict[str, Any], keep_intermediate: bool = False) -> Dict[str, Any]:
    """
    ...
    for field in _RENDERING_REQUIRED_FIELDS:
        if field in scene:
            simplified[field] = scene[field]
    
    # Optionally keep intermediate fields
    if keep_intermediate:
        for field in _INTERMEDIATE_FIELDS:
            if field in scene:
                simplified[field] = scene[field]
    
    # Note: floor_objects, wall_objects, small_objects, ceiling_objects are
    # already merged into objects, so we don't need to keep them separately
...
class Holodeck:
    def __init__(
        ...
        print(f"[Holodeck] Using device: {self.device}")
        ...
        self.clip_model = self.clip_model.to(self.device)  # Move to GPU
    ...
    def generate_scene(
        ...
        # Save scene (simplified by default, or full if save_simplified=False)
        scene_to_save = _simplify_scene(scene, keep_intermediate=not save_simplified) if save_simplified else scene
        compress_json.dump(
            scene_to_save,
        ...
        if generate_image:
            ...
            # Skip show() in headless environments (no display available)
            try:
                top_image.show()
            except Exception:
    ...
    def generate_scene_from_layout(
        self,
        scene,
        query: str,
        save_dir: str,
        ...
        random_selection=False,
        use_milp=False,
        start_from: str = "floor_objects",
        save_simplified=True,
    ) -> Tuple[Dict[str, Any], str]:
        """
        Generate scene from an intermediate layout design.
        
        This method allows you to provide a partially generated scene (e.g., with rooms, 
        walls, doors, windows, and selected_objects already defined) and continue generation
        from a specific step. This is useful when you want to revise the layout design
        at the module level rather than modifying the final JSON.
        
        Args:
            scene: Partially generated scene dictionary. Must contain at least:
                   - rooms (from generate_rooms)
                   - walls (from generate_walls)
                   - doors (from generate_doors)
                   - windows (from generate_windows)
                   - selected_objects (from select_objects)
            query: Original query string
            save_dir: Directory to save the scene
            used_assets: List of used asset IDs
            add_ceiling: Whether to add ceiling objects
            generate_image: Whether to generate top-down image
            generate_video: Whether to generate video
            add_time: Whether to add timestamp to folder name
            use_constraint: Whether to use constraints for object placement
            random_selection: Whether to use random object selection
            use_milp: Whether to use MILP solver
            start_from: Step to start from. Options:
                       - "floor_objects": Start from floor object placement (default)
                       - "wall_objects": Start from wall object placement
                       - "small_objects": Start from small object placement
                       - "ceiling_objects": Start from ceiling object placement
            save_simplified: If True (default), save only rendering-required fields.
                           If False, save full scene with all intermediate data.
        
        Returns:
            Tuple of (scene_dict, save_directory_path)
        """
        # Validate required scene components
        required_components = {
            "floor_objects": ["rooms", "walls", "doors", "windows", "selected_objects"],
            "wall_objects": ["rooms", "walls", "doors", "windows", "selected_objects", "floor_objects"],
            "small_objects": ["rooms", "walls", "doors", "windows", "selected_objects", "floor_objects", "wall_objects"],
            "ceiling_objects": ["rooms", "walls", "doors", "windows", "selected_objects", "floor_objects", "wall_objects", "small_objects"],
        }
        
        if start_from not in required_components:
            raise ValueError(f"Invalid start_from: {start_from}. Must be one of {list(required_components.keys())}")
        
        required = required_components[start_from]
        missing = [comp for comp in required if comp not in scene]
        if missing:
            raise ValueError(f"Scene is missing required components for start_from='{start_from}': {missing}")
        
        # Initialize query
        query = query.replace("_", " ")
        scene["query"] = query
        
        # Ensure open_walls exists (needed for floor object generation)
        if "open_walls" not in scene:
            scene["open_walls"] = scene.get("open_room_pairs", [])
        
        print(f"\n[Generate from Layout] Starting from step: {start_from}")
        print(f"Scene already contains: {list(scene.keys())}")
        
        # Continue generation from specified step
        if start_from == "floor_objects":
            # Generate floor objects
            self.floor_object_generator.use_milp = use_milp
            scene["floor_objects"] = self.floor_object_generator.generate_objects(
                scene, use_constraint=use_constraint
        ...
        elif start_from == "ceiling_objects":
            # Generate ceiling objects
            if add_ceiling:
                scene = self.generate_ceiling_objects(
                    scene,
