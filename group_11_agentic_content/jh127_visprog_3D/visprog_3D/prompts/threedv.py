...
""",
# For 3D reasoning
"""Question: What is the bounding box of the 3D scene (two images)?
Program:
POINTCLOUD0=RECON(image_list=IMAGE_LIST)
BBOX0=BOUND(pc=POINTCLOUD0)
FINAL_RESULT=RESULT(var=BBOX0)
""",
"""Question: What is size of this scene (two images)?
Program:
POINTCLOUD0=RECON(image_list=IMAGE_LIST)
BBOX0=BOUND(pc=POINTCLOUD0)
FINAL_RESULT=RESULT(var=BBOX0)
""",
"""Question: Locate the xbox controller in 3D (two images).
Program:
POINTCLOUD0=RECON(image_list=IMAGE_LIST)
PC0=IND(list=POINTCLOUD0,index=0)
IMAGE0=IND(list=IMAGE_LIST,index=0)
OBJ0=SEG(image=IMAGE0)
OBJ1=SELECT(image=IMAGE0,object=OBJ0,query='xbox controller',category='controller')
PC0=SELECTPC(pc=PC0,object=OBJ1)
PC1=IND(list=POINTCLOUD0,index=1)
IMAGE1=IND(list=IMAGE_LIST,index=1)
OBJ2=SEG(image=IMAGE1)
OBJ3=SELECT(image=IMAGE1,object=OBJ2,query='xbox controller',category='controller')
PC1=SELECTPC(pc=PC1,object=OBJ3)
PC2=MERGEPC(pc0=PC0,pc1=PC1)
BBOX0=BOUND(pc=PC2)
FINAL_RESULT=RESULT(var=BBOX0)
""",
"""Question: Remove the xbox controller in 3D (one images).
Program:
POINTCLOUD0=RECON(image_list=IMAGE_LIST)
PC0=IND(list=POINTCLOUD0,index=0)
IMAGE0=IND(list=IMAGE_LIST,index=0)
OBJ0=SEG(image=IMAGE0)
OBJ1=SELECT(image=IMAGE0,object=OBJ0,query='xbox controller',category='controller')
PC0=REMOVEPC(pc=PC0,object=OBJ1)
FINAL_RESULT=RESULT(var=PC0)
""",
"""Question: Delete the robot in 3D (three images).
Program:
POINTCLOUD0=RECON(image_list=IMAGE_LIST)
PC0=IND(list=POINTCLOUD0,index=0)
IMAGE0=IND(list=IMAGE_LIST,index=0)
OBJ0=SEG(image=IMAGE0)
OBJ1=SELECT(image=IMAGE0,object=OBJ0,query='robot',category='robot')
PC0=REMOVEPC(pc=PC0,object=OBJ1)
PC1=IND(list=POINTCLOUD0,index=1)
IMAGE1=IND(list=IMAGE_LIST,index=1)
OBJ2=SEG(image=IMAGE1)
OBJ3=SELECT(image=IMAGE1,object=OBJ2,query='robot',category='robot')
PC1=REMOVEPC(pc=PC1,object=OBJ3)
PC2=IND(list=POINTCLOUD0,index=2)
IMAGE2=IND(list=IMAGE_LIST,index=2)
OBJ4=SEG(image=IMAGE2)
OBJ5=SELECT(image=IMAGE1,object=OBJ4,query='robot',category='robot')
PC2=REMOVEPC(pc=PC2,object=OBJ5)
PC3=MERGEPC(pc0=PC0,pc1=PC1)
PC3=MERGEPC(pc0=PC3,pc1=PC2)
FINAL_RESULT=RESULT(var=PC3)
""",
"""Question: Compute distance between apple and orange in 3D (two images).
Program:
POINTCLOUD0=RECON(image_list=IMAGE_LIST)
PC0=IND(list=POINTCLOUD0,index=0)
IMAGE0=IND(list=IMAGE_LIST,index=0)
OBJ0=SEG(image=IMAGE0)
OBJ1=SELECT(image=IMAGE0,object=OBJ0,query='apple',category='fruit')
PC0=SELECTPC(pc=PC0,object=OBJ1)
PC1=IND(list=POINTCLOUD0,index=1)
IMAGE1=IND(list=IMAGE_LIST,index=1)
OBJ2=SEG(image=IMAGE1)
OBJ3=SELECT(image=IMAGE1,object=OBJ2,query='orange',category='fruit')
PC1=SELECTPC(pc=PC1,object=OBJ3)
DIST=DISTPC(pc0=PC0,pc1=PC1)
FINAL_RESULT=RESULT(var=DIST)
""",
]
