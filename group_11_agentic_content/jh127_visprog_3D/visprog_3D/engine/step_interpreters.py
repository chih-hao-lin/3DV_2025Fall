...
class ReconInterpreter():
    ...
    def __init__(self):
        print(f'Registering {self.step_name} step')
        device = "cuda"
        self.pipe = VGGT.from_pretrained("facebook/VGGT-1B").to(device)

    def parse(self,prog_step):
        parse_result = parse_step(prog_step.prog_str)
        step_name = parse_result['step_name']
        img_list_var = parse_result['args']['image_list']
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        return img_list_var,output_var

    def reconstruct(self,img_list):
        device = "cuda"
        torch_dtype = torch.float16
        images = torch.stack([transforms.ToTensor()(img).to(torch_dtype) for img in img_list],dim=0)  # (B, H, W, 3)
        images = images.to(device)

        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=torch_dtype):
                # Predict attributes including cameras, depth maps, and point maps.
                predictions = self.pipe(images)

        extrinsic, intrinsic = pose_encoding_to_extri_intri(predictions['pose_enc'], images.shape[-2:])
        extrinsic = extrinsic[0]
        intrinsic = intrinsic[0]
        
        depth = predictions['depth'][0]  # (B, H, W)

        conf_threshold = 1.0
        # conf = predictions['world_points_conf'][0]  # (B, H, W)
        conf = predictions['depth_conf'][0]  # (B, H, W)
        mask = conf >= conf_threshold
        mask = mask.cpu().numpy()
        
        # pc = predictions['world_points'][0]
        pc = unproject_depth_map_to_point_map(depth, extrinsic, intrinsic)
        pc[~mask] = np.nan
        color = (images.cpu().numpy().transpose(0,2,3,1))
        pc = np.concatenate([pc, color], axis=-1)  # (B, H, W, 6)
        return pc
    
    def render_pointcloud(self,pointcloud):
        if len(pointcloud.shape)==3:
            pointcloud = pointcloud[np.newaxis,:,:,:]
        # pointcloud: (B, H, W, 6)
        mask = ~np.isnan(pointcloud[:,:,:,0])
        pc = pointcloud[mask]
        pc_klampt = numpy_convert.from_numpy(pc[:,:3], type='PointCloud')
        pc_klampt.setColors(pc[:,3:6], color_format=('r','g','b'))
        vis.add("pointcloud", pc_klampt)
        vis.show()
        while vis.shown():
            time.sleep(0.1)
        vis.show()
    ...
    def html(self,img_list,output_var,output):
        step_name = html_step_name(self.step_name)
        imgs = ""
        for img in img_list:
            imgs += html_embed_image(img,300) + ','
        imgs = imgs[:-1]
        output_var = html_var_name(output_var)
        img_arg = html_arg_name('image_list')
        output_img = self.render_pointcloud(output)
        output_img = html_embed_image(output_img,300)
        return f"""{output_var}={step_name}({img_arg}={imgs})={output_img}"""

    def execute(self,prog_step,inspect=False):
        img_list_var,output_var = self.parse(prog_step)
        img_list = prog_step.state[img_list_var]
        pointcloud = self.reconstruct(img_list)
        prog_step.state[output_var] = pointcloud
        if inspect:
            html_str = self.html(img_list, output_var, pointcloud)
            return pointcloud, html_str
        return pointcloud
...
class BoundInterpreter():
    ...
    def parse(self,prog_step):
        parse_result = parse_step(prog_step.prog_str)
        step_name = parse_result['step_name']
        pc_var = parse_result['args']['pc']
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        return pc_var,output_var
    
    def bound_pointcloud(self,pointcloud):
        if len(pointcloud.shape)==3:
            pointcloud = pointcloud[np.newaxis,:,:,:]
        # pointcloud: (B, H, W, 3)
        mask = ~np.isnan(pointcloud[:,:, :,0])
    ...
    def render_bbox(self,bbox, pointcloud):
        if len(pointcloud.shape)==3:
            pointcloud = pointcloud[np.newaxis,:,:,:]
        # pointcloud: (B, H, W, 6)
        mask = ~np.isnan(pointcloud[:,:,:,0])
        ...
        pc_klampt = numpy_convert.from_numpy(pc[:,:3], type='PointCloud')
        pc_klampt.setColors(pc[:,3:6], color_format=('r','g','b'))
        box = klampt.model.create.bbox(*bbox)
        vis.add("pointcloud", pc_klampt)
        vis.add("bbox", box, color=(1,0,0,0.5))
        vis.show()
        while vis.shown():
            time.sleep(0.1)
        vis.show()
    ...
    def html(self,pc_var,pc,output_var,output):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        pc_arg = html_arg_name('pc')
        pc_var = html_var_name(pc_var)
        output_img = self.render_bbox(output, pc)
        output_img = html_embed_image(output_img,300)
        return f"""{output_var}={step_name}({pc_arg}={pc_var})={output_img}"""

    def execute(self,prog_step,inspect=False):
        pc_var,output_var = self.parse(prog_step)
        pc = prog_step.state[pc_var]
        bbox = self.bound_pointcloud(pc)
        prog_step.state[output_var] = bbox
        if inspect:
            html_str = self.html(pc_var, pc, output_var, bbox)
            return bbox, html_str
        return bbox
...
class IndexInterpreter():
    ...
    def parse(self,prog_step):
        parse_result = parse_step(prog_step.prog_str)
        step_name = parse_result['step_name']
        list_var = parse_result['args']['list']
        index = int(parse_result['args']['index'])
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        return list_var,index,output_var

    def html(self,list_var,index,output_var,output):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        list_arg = html_arg_name('list')
        index_arg = html_arg_name('index')
        if isinstance(output, Image.Image):
            output = html_embed_image(output,300)
        return f"""{output_var}={step_name}({list_arg}={list_var},{index_arg}={index})={output}"""

    def execute(self,prog_step,inspect=False):
        list_var,index,output_var = self.parse(prog_step)
        l = prog_step.state[list_var]
        elem = l[index]
        prog_step.state[output_var] = elem
        if inspect:
            html_str = self.html(list_var, index, output_var, elem)
            return elem, html_str
        return elem

class SelectPCInterpreter():
    step_name = 'SELECTPC'
    ...
    def parse(self,prog_step):
        ...
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        return pc_var,obj_var,output_var
    ...
    def html(self,pc_var,obj_var,output_var,output):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        pc_arg = html_arg_name('pc')
        pc_var = html_var_name(pc_var)
        ...
        obj_var = html_var_name(obj_var)
        output = self.render_pointcloud(output)
        output = html_embed_image(output,300)
        return f"""{output_var}={step_name}({pc_arg}={pc_var},{object_arg}={obj_var})={output}"""

    def execute(self,prog_step,inspect=False):
        pc_var,obj_var,output_var = self.parse(prog_step)
        pc = prog_step.state[pc_var]
        objs = prog_step.state[obj_var]
        mask = objs[0]['mask']
        selected_pc = pc.copy()
        selected_pc[~(mask.astype(bool))] = np.nan
        prog_step.state[output_var] = selected_pc

        if inspect:
            html_str = self.html(pc_var, obj_var, output_var, selected_pc)
            return selected_pc, html_str
        return selected_pc
    

class RemovePCInterpreter():
    step_name = 'REMOVEPC'
...
class MergePCInterpreter():
    ...
    def parse(self,prog_step):
        parse_result = parse_step(prog_step.prog_str)
        step_name = parse_result['step_name']
        pc0_var = parse_result['args']['pc0']
        pc1_var = parse_result['args']['pc1']
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        return pc0_var,pc1_var,output_var
    ...
    def html(self,pc0_var,pc1_var,output_var,output):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        pc0_arg = html_arg_name('pc0')
        pc0_var = html_var_name(pc0_var)
        pc1_arg = html_arg_name('pc1')
        pc1_var = html_var_name(pc1_var)
        output = self.render_pointcloud(output)
        output = html_embed_image(output,300)
        return f"""{output_var}={step_name}({pc0_arg}={pc0_var},{pc1_arg}={pc1_var})={output}"""

    def execute(self,prog_step,inspect=False):
        pc0_var,pc1_var,output_var = self.parse(prog_step)
        pc0 = prog_step.state[pc0_var]
        pc1 = prog_step.state[pc1_var]
        pc = np.concatenate([pc0,pc1],axis=0)
        prog_step.state[output_var] = pc

        if inspect:
            html_str = self.html(pc0_var, pc1_var, output_var, pc)
            return pc, html_str
        return pc

class DistPCInterpreter():
    step_name = 'DISTPC'
    ...
    def execute(self,prog_step,inspect=False):
        ...
        pc0 = prog_step.state[pc0_var]
        pc1 = prog_step.state[pc1_var]
        # compute distance between two point clouds centers
        if len(pc0.shape)==3:
            pc0 = pc0[np.newaxis,:,:,:]
        if len(pc1.shape)==3:
            pc1 = pc1[np.newaxis,:,:,:]
        mask0 = ~np.isnan(pc0[:,:,:,0])
        pc0_pts = pc0[mask0]
        center0 = np.mean(pc0_pts[:,:3],axis=0)
        mask1 = ~np.isnan(pc1[:,:,:,0])
        pc1_pts = pc1[mask1]
        center1 = np.mean(pc1_pts[:,:3],axis=0)
        dist = np.linalg.norm(center0 - center1)
        prog_step.state[output_var] = dist


        if inspect:
            html_str = self.html(pc0_var, pc1_var, output_var, dist)
            return dist, html_str
        return dist
...
def register_step_interpreters(dataset='nlvr'):
    if dataset=='nlvr':
        ...
        return dict(
            ...
            EVAL=EvalInterpreter(),
            RESULT=ResultInterpreter(),
            RECON=ReconInterpreter(),
            BOUND=BoundInterpreter(),
            IND=IndexInterpreter(),
            SELECTPC=SelectPCInterpreter(),
            REMOVEPC=RemovePCInterpreter(),
            MERGEPC=MergePCInterpreter(),
            DISTPC=DistPCInterpreter()
        )