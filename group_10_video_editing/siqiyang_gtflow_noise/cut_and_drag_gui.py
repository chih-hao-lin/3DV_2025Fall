...
from noise_warp import regaussianize, resize_noise


LAYER_CONFIG_FILENAME = "layer_config.json"


def layer_config_path(folder):
    return path_join(folder, LAYER_CONFIG_FILENAME)


def load_project_metadata(folder):
    config_path = layer_config_path(folder)
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"No saved animation data found at {config_path}")
    with open(config_path, "r", encoding="utf-8") as f:
        return json.load(f)


def save_project_metadata(folder, data):
    config_path = layer_config_path(folder)
...
if __name__ == "__main__":
    ...
    reuse_existing = input_yes_no(
        fansi("Load existing project data?", "blue cyan", "italic bold underlined")
    )

    existing_layer_metadata = []
    current_layer_metadata = []
    title = None

    if reuse_existing:
        existing_folder = input_conditional(
            fansi("Existing project folder path", "blue cyan", "italic bold underlined"),
            lambda x: folder_exists(x.strip()),
        ).strip()
        output_folder = existing_folder
        existing_metadata = load_project_metadata(existing_folder)
        existing_layer_metadata = existing_metadata.get("layers", [])
        if not existing_layer_metadata:
            raise RuntimeError("No saved layers found in the existing project.")
        title = existing_metadata.get("title", get_file_name(existing_folder))
        HEIGHT = int(existing_metadata.get("height", 480))
        WIDTH = int(existing_metadata.get("width", 720))
        stored_image_path = existing_metadata.get("image_path")
        if stored_image_path and (is_a_file(stored_image_path) or is_valid_url(stored_image_path)):
            print("Using saved path: " + fansi_highlight_path(stored_image_path))
        ...
        rp.fansi_print(
            "PRO TIP: Use this website to help write your captions: https://huggingface.co/spaces/THUDM/CogVideoX-5B-Space",
            "blue cyan",
        )
        prompt = input_default(
            fansi("Input the video caption >>> ", "blue cyan", "bold"),
            existing_metadata.get("prompt", ""),
        )
        image = resize_image_to_hold(image, height=HEIGHT, width=WIDTH)
        image = crop_image(image, height=HEIGHT, width=WIDTH, origin="center")
        num_layers = len(existing_layer_metadata)
    ...
    for layer_num in range(num_layers):
        if reuse_existing:
            fansi_print(f'Loading saved layer #{layer_num+1} of {num_layers}', 'yellow orange', 'bold')
            layer_info = existing_layer_metadata[layer_num]
            noise_filename = layer_info.get("noise_file")
            if not noise_filename:
                raise RuntimeError(f"Layer #{layer_num+1} is missing noise_file metadata.")
            noise_path = path_join(output_folder, noise_filename)
            if not os.path.exists(noise_path):
                raise FileNotFoundError(f"Missing noise file for layer #{layer_num+1}: {noise_path}")
            layer_noise = np.load(noise_path)
            polygon_array = np.array(layer_info["polygon"], dtype=float)
            path_array = np.array(layer_info["path"], dtype=float)
            scales_array = np.array(layer_info["scales"], dtype=float)
            rotations_array = np.array(layer_info["rotations"], dtype=float)
        else:
            ...
            noise_filename = f"layer_{layer_num:03d}_noise.npy"
            np.save(path_join(output_folder, noise_filename), layer_noise)
            polygon = select_polygon(image)
            animation_path, animation_scales, animation_rotations = select_path(image, polygon)
            polygon_array = np.array(polygon, dtype=float)
            path_array = np.array(animation_path, dtype=float)
            scales_array = np.array(animation_scales, dtype=float)
            rotations_array = np.array(animation_rotations, dtype=float)

        polygon_points = [tuple(pt) for pt in polygon_array.tolist()]

        animation_output = animate_polygon(
            image,
            polygon_points,
            path_array,
            scales_array,
        ...
        if reuse_existing:
            noise_filename = layer_info["noise_file"]

        metadata_entry = {
            "polygon": polygon_array.tolist(),
            "path": path_array.tolist(),
            "scales": [float(x) for x in scales_array.tolist()],
            "rotations": [float(x) for x in rotations_array.tolist()],
            "noise_file": noise_filename,
        }
        current_layer_metadata.append(metadata_entry)
    ...
    #     torch_noises[i]=regaussianize(torch_noises[i])[0]
    #     small_torch_noise=resize_noise(torch_noises[i],(480//8,720//8))
    ...
    fansi_print("Using same noise for all frames...", 'yellow green', 'bold italic')

    # 初始化随机噪声
    base_noise = np.random.randn(1, HEIGHT, WIDTH, 16)
    # 复制成 49 帧
    output_noises = np.repeat(base_noise, 49, axis=0)

    # （可选）可视化几帧噪声
    for i in [0, 24, 48]:
        display_image(output_noises[i, :, :, :3] / 5 + 0.5)

    # 转换成 torch tensor 并重排维度
    torch_noises = torch.tensor(output_noises)
    torch_noises = einops.rearrange(torch_noises, 'F H W C -> F C H W')

    # Regaussianize + Downsample
    small_torch_noises = []
    for i in eta(range(49), title='Regaussianizing'):
        torch_noises[i] = regaussianize(torch_noises[i])[0]
        small_torch_noise = resize_noise(torch_noises[i], (480 // 8, 720 // 8))
        small_torch_noises.append(small_torch_noise)
        # 可视化再高斯化后的图像
        display_image(as_numpy_image(torch_noises[i, :3]) / 5 + 0.5)

    small_torch_noises = torch.stack(small_torch_noises)
    ...
    project_metadata = {
        ...
        "height": int(HEIGHT),
        "width": int(WIDTH),
        "layers": current_layer_metadata,
    }
    save_project_metadata(output_folder, project_metadata)
