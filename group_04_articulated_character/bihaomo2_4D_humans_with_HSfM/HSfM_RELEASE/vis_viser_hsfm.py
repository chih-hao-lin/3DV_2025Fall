...
def show_optimization_results(world_env, body_model_name, human_params, smplx_layer):
    ...
    for human_name, optim_target_dict in human_params.items():
        if body_model_name == 'smplx':
            ...
            bp = optim_target_dict['body_pose']
            if bp.ndim == 2 and bp.shape[-1] == 207:           # (B, 207)
                B = bp.shape[0]
                body_pose = bp.view(B, 23, 9).reshape(B, 23, 3, 3)
            else:
                body_pose = optim_target_dict['body_pose'][None, :, :, :] # (1, 23, 3, 3)

            go = optim_target_dict['global_orient']
            if go.ndim == 2 and go.shape[-1] == 9:           # (B, 207)
                B = go.shape[0]
                global_orient = go.reshape(B, 1, 3, 3)
            else:
                global_orient = optim_target_dict['global_orient'][None, :, :] # (1, 1, 3, 3)
            
            betas = optim_target_dict['betas']
            if betas.ndim == 1:
                betas = betas[None, :]
