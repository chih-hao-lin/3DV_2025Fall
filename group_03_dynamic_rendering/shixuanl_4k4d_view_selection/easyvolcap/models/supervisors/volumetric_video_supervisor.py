    ...
    def compute_loss(self, output: dotdict, batch: dotdict, loss: torch.Tensor, scalar_stats: dotdict, image_stats: dotdict):
        ...
        if 'rgb_map' in output and \
           ...
           batch.meta.n_rays[0].item() == -1:
            ...
            # print(output.rgb_map.shape, batch.rgb.shape, output.bg_color.shape, batch.msk.shape)
            # if output.rgb_map.shape != batch.rgb.shape:
            #     import cv2
            #     def resize_object(obj):
            #         target_h, target_w = output.rgb_map.shape[-3:-1]  # get height, width
            #         resized = []
            #         for i in range(obj.shape[-1]):  # loop over batch
            #             img = obj[:, :, i]
            #             img_np = img.cpu().numpy()
            #             resized_img = cv2.resize(img_np, (target_w, target_h), interpolation=cv2.INTER_LINEAR)
            #             resized.append(resized_img)

            #         # Convert back to torch tensor
            #         return torch.from_numpy(np.stack(resized, axis=-1)).to(batch.rgb.device, dtype=batch.rgb.dtype)
            #     batch.rgb = resize_object(batch.rgb)
            #     batch.msk = resize_object(batch.msk)
                # output.bg_color = resize_object(output.bg_color)
            # print(output.rgb_map.shape, batch.rgb.shape, output.bg_color.shape, batch.msk.shape)
