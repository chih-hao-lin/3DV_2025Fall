...
class VolumetricVideoRunner:  # a plain and simple object controlling the training loop
    def __init__(self,
                 ...
                 # Attribution
                 attribution_model: VolumetricVideoModel = None,
    ...
    # Single epoch testing api
    def tests(self):  # from begin epoch
        epoch = self.load_network()
        # load attribution model also
        # load_network(model=self.attribution_model,  # only loading the network, without recorder?
        #              model_dir=self.trained_model,
        #              strict=self.strict,
        #             )
        self.tests_epoch(epoch)

    def tests_epoch(self, epoch: int):
        tests_generator = self.test_generator_with_data_selection(epoch, -1)  # nevel yield (special logic)
        for _ in tests_generator: pass  # the actual calling

    def test_generator_with_data_selection(self, epoch: int, yield_every: int = 1):
        import os, psutil
        ...
        self.attribution_model = self.attribution_model
        # for n, p in self.attribution_model.named_parameters():  # selected layers
        #     if "appearance" in n:
        #         p.requires_grad = True
        #     else:
        #         p.requires_grad = False
        trainable = [n for n,p in self.attribution_model.named_parameters() if p.requires_grad]
        print("trainable params:", len(trainable))
        print(len(self.dataloader), len(self.val_dataloader))

        # We do not count mask loss
        # self.attribution_model.supervisor.supervisors[0].msk_loss_weight = 0.0
        # self.attribution_model.supervisor.supervisors[0].msk_mse_weight = 0.0

        def grad_dot(grad_x, grad_y):
            res = 0
            for k in grad_x:
                assert k in grad_y
                res += torch.sum(grad_x[k] * grad_y[k])
            return res
        
        def grad_sum(grad_x, grad_y):
            for k in grad_x:
                assert k in grad_y
                grad_x[k] += grad_y[k]
            return grad_x

        def accumulate_gradients_and_zero_out(model):
            res = {}
            for n, p in model.named_parameters():
                if p.grad is not None:
                    res[n] = p.grad.detach().cpu()
                    p.grad = None
            return res
        ...
        # Perform data attribution on views
        all_gradients = {}
        total_gradients = None
        for index, batch in enumerate(
            tqdm(self.dataloader,
                 disable=not self.print_test_progress,
                 desc="Selecting Influential Views")
            ):
            iter = epoch * self.ep_iter - 1  # some indexing trick
            batch = add_iter(batch, iter, self.total_iter)  # is this bad naming
            batch = to_cuda(batch)  # cpu -> cuda, note that DDP will move all cpu tensors to cuda as well
            with torch.cuda.amp.autocast(enabled=self.train_use_amp), torch.enable_grad():
                output: dotdict = self.attribution_model(batch)
                loss: torch.Tensor = output.loss.mean() # calculate loss here
                scalar_stats = self.evaluator.evaluate(output, batch)
                image_stats = self.visualizer.visualize(output, batch)

            self.scaler.scale(loss).backward()  # obtain gradients
            all_gradients[index] = accumulate_gradients_and_zero_out(self.attribution_model)
            if total_gradients is None:
                total_gradients = copy_tensor_dict(all_gradients[index])
            else:
                total_gradients = grad_sum(total_gradients, all_gradients[index])

            print(index, self.dataloader.dataset.get_indices(index))
            print("Memory (GB):", process.memory_info().rss / 1024**3)  # RSS = resident set size

        num_views = len(self.dataloader.dataset.view_sample)
        all_scores = torch.zeros(num_views)
        for i in tqdm(range(num_views), disable=not self.print_test_progress, desc="calculating Scores"):
            # group by camera / views
            view_index, latent_index, camera_index, frame_index = self.dataloader.dataset.get_indices(i)
            all_scores[view_index] += grad_dot(total_gradients, all_gradients[i])

        ranking = torch.argsort(all_scores, descending=True)
        topk = 30
        selected_indices = ranking[:topk]

        # update model & val_dataloader
        # self.val_dataloader.dataset.src_ixts = self.val_dataloader.dataset.src_ixts[selected_indices]
        # self.val_dataloader.dataset.src_exts = self.val_dataloader.dataset.src_exts[selected_indices]
        # self.val_dataloader.dataset.n_views = topk
        from easyvolcap.engine import RUNNERS, MODELS, DATALOADERS
        self.val_dataloader_cfg.dataset_cfg["view_sample"] = [
            i for i in self.val_dataloader_cfg.dataset_cfg["view_sample"] if i in selected_indices
        ]
        self.val_dataloader_cfg.dataset_cfg["ratio"] = [
            r for i, r in enumerate(self.val_dataloader_cfg.dataset_cfg["ratio"]) if i in selected_indices
        ]
        new_val_dataloader = DATALOADERS.build(self.val_dataloader_cfg)  # reuse the validataion
        self.model.sampler.recharge(new_val_dataloader.dataset)
        self.model.sampler.n_srcs = topk
        self.model.n_srcs = topk
        ...
        for index, batch in enumerate(tqdm(self.val_dataloader, disable=not self.print_test_progress)):
            ...
            batch = add_iter(batch, iter, self.total_iter)  # is this bad naming
            batch = to_cuda(batch)  # cpu -> cuda, note that DDP will move all cpu tensors to cuda as well
            with torch.cuda.amp.autocast(enabled=self.test_use_amp):
                output: dotdict = self.model(batch)
                scalar_stats = self.evaluator.evaluate(output, batch)
                image_stats = self.visualizer.visualize(output, batch)
    ...
    # Single epoch testing api
    def testr(self):  # from begin epoch
        epoch = self.load_network()
        self.testr_epoch(epoch)

    def testr_epoch(self, epoch: int):
        tests_generator = self.test_generator_random(epoch, -1)  # nevel yield (special logic)
        for _ in tests_generator: pass  # the actual calling

    def test_generator_random(self, epoch: int, yield_every: int = 1):
        # random ranking
        torch.manual_seed(42)
        ranking = torch.randperm(60)
        topk = 30
        selected_indices = ranking[:topk]
