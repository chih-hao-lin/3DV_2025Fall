...
def tests(
    model_cfg: dotdict = dotdict(type="VolumetricVideoModel"),
    attribution_model_cfg: dotdict = dotdict(type="VolumetricVideoModel"),
    ...
    **kwargs,
    ...
    # print(val_dataloader_cfg)

    # Construct other parts of the training process
    import copy
    attribution_dataloader_cfg = copy.deepcopy(val_dataloader_cfg)
    attribution_dataloader_cfg["dataset_cfg"]["type"] = "ImageBasedDataset"
    attribution_dataloader_cfg["dataset_cfg"]["append_gt_prob"] = 1.0
    attribution_dataloader_cfg["dataset_cfg"]["render_ratio"] = 0.1  # low quality images

    dataloader: "VolumetricVideoDataloader" = DATALOADERS.build(attribution_dataloader_cfg)
    ...
    attribution_model: "VolumetricVideoModel" = MODELS.build(attribution_model_cfg)
    attribution_model = attribution_model.to(base_device, non_blocking=True)
    ...
    # Just run, no gossip
    launcher(**kwargs, runner_function=runner.tests, runner_object=runner)
...
def testr(
    ...
    **kwargs,
    ...
    # Just run, no gossip
    launcher(**kwargs, runner_function=runner.testr, runner_object=runner)
