...
from gns.render_rollout import Render, TYPE_TO_COLOR


from gns import learned_simulator_trans
import random
import wandb
...
flags.DEFINE_integer('ntraining_steps', int(2E6), help='Number of training steps.') #2E7
flags.DEFINE_integer('validation_interval', 5000, help='Validation interval. Set `None` if validation loss is not needed') # None
flags.DEFINE_integer('nsave_steps', int(25000), help='Number of steps at which to save the model.')
...
flags.DEFINE_string("model_type", 'transformer', help="Model type to use - ['transformer', 'gnn'].")
flags.DEFINE_boolean("wandb", False, help="Use wandb to log training stats.")
...
def predict(device: str):
  ...
  simulator = _get_simulator(metadata, FLAGS.noise_std, FLAGS.noise_std, device, FLAGS.model_type)

  # Load simulator
  if os.path.exists(FLAGS.model_path + FLAGS.model_file):
    if(FLAGS.model_file == None):
      largest_step = -1
      for file in sorted(os.listdir(flags["model_path"])):
        if file.startswith("model") and file.endswith(".pt"):
          curr_step = int(re.findall(r'model-(\d+).pt', file)[0])
          if curr_step > largest_step:
            largest_step = curr_step
      file_path = flags["model_path"] + f"model-{largest_step}.pt"
      print(f"Loading model from {file_path}")
      simulator.load(file_path)
    else:
      file_path = FLAGS.model_path + FLAGS.model_file
      print(f"Loading model from {file_path}")
      simulator.load(file_path)
  else:
  ...
  with torch.no_grad():
    for example_i, features in enumerate(ds):
      ...
      if num_samples >= 10:
        break
      # break # test code, only do one example

  print("Mean loss on rollout prediction: {}".format(
      torch.mean(torch.cat(eval_loss))))
  
def training_predict(device: str, flags=None):
  """Predict rollouts.
  ...
  if os.path.exists(flags["model_path"]):
    # File is saved as model-step.pt, so get the latest step, there are other files in the directory.
    largest_step = -1
    for file in sorted(os.listdir(flags["model_path"])):
      if file.startswith("model") and file.endswith(".pt"):
        curr_step = int(re.findall(r'model-(\d+).pt', file)[0])
  ...
  # Use `valid`` set for eval mode if not use `test`
  split = 'test' if (flags["mode"] == 'rollout' or (not os.path.isfile(f"{flags['data_path']}valid.npz"))) else 'valid'

  print(f"Using {split} set for evaluation")
  # Get dataset
  ds = data_loader.get_data_loader_by_trajectories(path=f"{flags['data_path']}{split}.npz")
  # See if our dataset has material property as feature
  if len(ds.dataset._data[0]) == 3:  # `ds` has (positions, particle_type, material_property)
  ...
  with torch.no_grad():
    for example_i, features in enumerate(ds):
      ...
      if flags["mode"] == 'rollout':
        example_rollout['metadata'] = metadata
        example_rollout['loss'] = loss.mean()
        filename = f'{flags["output_filename"]}_iter{largest_step}_ex{example_i}.pkl'
        filename = os.path.join(flags["output_path"], filename)
...
def train(rank, flags, world_size, device):
  ...
  if flags["wandb"]:
    ...
    run = wandb.init(
      ...
      config={
          ...
          "architecture": flags["model_type"],
          "dataset": flags["data_path"].split("/")[-2],
          "batch_size": flags["batch_size"],
          "noise_std": flags["noise_std"],
          "description": "Training Orig GNS model",
  ...
  print("Training with metadata:", metadata)

  # Get simulator and optimizer
  if device == torch.device("cuda"):
    serial_simulator = _get_simulator(metadata, flags["noise_std"], flags["noise_std"], rank, flags["model_type"])
  ...
  # get number of parameters in each layer and total
  print("Model parameters in each layer:")
  total_params = 0
  ...
  print("Number of training examples: {}".format(len(dl.dataset)))
  if flags["validation_interval"] is not None:
      print("Number of validation examples: {}".format(len(dl_valid.dataset)))
      ...
      for example in dl:  
        ...
        if rank == 0 or device == torch.device("cpu"):
          if step % flags["nsave_steps"] == 0:
            ...
            # rollout every save step
            flags["mode"] = "rollout"
            _ = training_predict(device, flags)

            render = Render(input_dir=flags["output_path"], input_name=f"rollout_iter{step}_ex0",)
...
def _get_simulator(
  ...
  if model_type == 'transformer':
    simulator = learned_simulator_trans.LearnedSimulatorTransformer(
