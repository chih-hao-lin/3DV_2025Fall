...
from gns import transformer_network
...
class LearnedSimulatorTransformer(nn.Module):
  """Learned simulator from https://arxiv.org/pdf/2002.09405.pdf."""
  ...
  def __init__(
    ...
    """
    super(LearnedSimulatorTransformer, self).__init__()
    self._boundaries = boundaries
    self._connectivity_radius = connectivity_radius
    self._normalization_stats = normalization_stats
    self._nparticle_types = nparticle_types
    ...
    # Initialize the EncodeProcessDecode
    self._encode_process_decode = transformer_network.EncodeProcessDecode(
        nnode_in_features=nnode_in,
        nnode_out_features=particle_dimensions,
        nedge_in_features=nedge_in,
        latent_dim=latent_dim,
  ...
  def predict_positions(
    ...
    most_recent_position = current_positions[:, -1]  # (n_nodes, 2)
    predicted_normalized_acceleration = self._encode_process_decode(
        node_features, edge_index, edge_features, most_recent_position)
    next_positions = self._decoder_postprocessor(
        predicted_normalized_acceleration, current_positions)
    return next_positions
  ...
  def predict_accelerations(
    ...
    most_recent_position = noisy_position_sequence[:, -1]  # (n_nodes, 2)
    predicted_normalized_acceleration = self._encode_process_decode(
        node_features, edge_index, edge_features, most_recent_position)
