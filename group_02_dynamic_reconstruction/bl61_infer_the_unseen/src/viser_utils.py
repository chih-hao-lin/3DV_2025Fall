...
def request_share_url_standalone(server, timeout=30):
    """
    Standalone function to request a share URL from a viser server.
    
    Args:
        server: viser.ViserServer instance
        timeout: Maximum time to wait for URL generation (seconds)
    ...
    try:
        ...
        print("Requesting share URL from viser server...")
        share_url = server.request_share_url()
        
        # Wait for URL generation with timeout
        while share_url is None and (time.time() - start_time) < timeout:
            time.sleep(1)
            share_url = server.request_share_url()
        ...
        if share_url:
            ...
            try:
                ...
                print("  Install 'pyperclip' for automatic clipboard copying")
            except Exception:
                pass
                
            return share_url
        else:
            print("✗ Failed to generate share URL within timeout period")
            return None
            
    except Exception as e:
        print(f"✗ Error requesting share URL: {e}")
        return None
...
class PointCloudViewer:
    def __init__(
        ...
        gui_share_url = self.server.gui.add_button(
            "Request Share URL",
            hint="Generate a shareable URL for this visualization"
        )
        
        # Text field to display the share URL
        self.share_url_display = self.server.gui.add_text(
            "Share URL:", 
            "Click 'Request Share URL' to generate a shareable link"
        )

        @gui_share_url.on_click
        def _(event: viser.GuiEvent) -> None:
            try:
                # Request share URL from viser server
                share_url = self.server.request_share_url()
                if share_url:
                    self.share_url_display.value = share_url
                    print(f"Share URL generated: {share_url}")
                    print(f"Share this URL with others to view the 3D scene: {share_url}")
                    
                    # Optionally try to copy to clipboard (if pyperclip is available)
                    try:
                        import pyperclip
                        pyperclip.copy(share_url)
                        print("URL copied to clipboard!")
                    except ImportError:
                        print("Install 'pyperclip' to automatically copy URLs to clipboard")
                    except Exception:
                        print("Could not copy to clipboard")
                        
                else:
                    self.share_url_display.value = "Failed to generate share URL - server may not support sharing"
                    print("Failed to generate share URL - make sure the viser server supports sharing")
            except Exception as e:
                error_msg = f"Error generating share URL: {str(e)}"
                self.share_url_display.value = error_msg
                print(error_msg)
                print("Note: Share URL functionality requires viser server to be running with sharing enabled")
    ...
    def get_share_url(self, timeout=30):
        """
        Get a shareable URL for this visualization.
        ...
        Returns:
            str: Share URL if successful, None otherwise
        """
        return request_share_url_standalone(self.server, timeout)
