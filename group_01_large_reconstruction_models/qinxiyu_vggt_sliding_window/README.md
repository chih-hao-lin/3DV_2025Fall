# VGGT Sliding Window 
### Hacker: Qinxi Yu (qinxiyu2@illinois.edu)

### Set up
follow same setup as original vggt

### How to run?
A example long-seq data incluidng 174 images is located at examples/long-seq/images.
To visualize, 
```
python demo_viser.py --window_size 30 --stride 20 --image_folder examples/long-seq/images
```

This repo is a modified version of origianl VGGT. Original VGGT can process up to 40-60 images on a RTX 4090 due to memory constraint. In this modified version, sliding-window approach is taken to handle it. Each time a window of images are processed by VGGT, the predictions are proccessed to transform to the world frame (the frame of the first camera). And the final result is aggregated.

Additional long-seq data is from BlendMVS 
